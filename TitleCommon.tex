\newcommand{\VSTUTitleHeading}{
Министерство образования и науки Российской Федерации\\
Федеральное государственное бюджетное образовательное учреждение высшего образования\\
«Волгоградский государственный технический университет»\\
}

% подчеркивание с подписью/датой и подчеркивание с ФИО
% #1 - длина подчеркивания
% #2 - подстрочная подпись
% #3 - надстрочная подпись
\newcommand{\VSTUUnderlinedField}[3]{%
$\underset{\text{(#2)}}{\uline{\makebox[#1]{#3}}}$%
}%

% Box <<согласовано>>
% Поскольку в бланке это поле изначально пустое,
% используем костыль \vphantom{рЙ}, чтобы подогнать высоту.
\newcommand{\VSTUTitleHeadAgreement}{
\begin{minipage}[t]{12.5em}
Согласовано\\
\VSTUUnderlinedField{\textwidth}{должность гл. специалиста предприятия}{\vphantom{рЙ}\VSTUFacilityExpertPost}\\
\VSTUUnderlinedField{2cm}{подпись}{\vphantom{рЙ \VSTUFacilityExpertDegreeAndName}}\hfill\VSTUUnderlinedField{4cm}{инициалы, фамилия}{\vphantom{рЙ}\VSTUFacilityExpertDegreeAndName}\\
\vphantom{Й}<<\makebox[1.5cm]{\hrulefill}>> \makebox[3cm]{\hrulefill} \the\year
\end{minipage}
}

% Box с подписью босса
\newcommand{\VSTUTitleHeadApproval}{
\begin{minipage}[t]{14.6em}
Утверждаю\\
\vspace{11pt}\VSTUHeadOfDepartmentPost, \VSTUHeadOfDepartmentDegree\\
\vspace{3.5pt}\VSTUUnderlinedField{2.5cm}{подпись}{\vphantom{\VSTUHeadOfDepartmentName}}\hfill\VSTUUnderlinedField{4.5cm}{инициалы, фамилия}{\VSTUHeadOfDepartmentName}\\
<<\makebox[1.5cm]{\hrulefill}>> \makebox[3.5cm]{\hrulefill} \the\year
\end{minipage}
}

% Box с подписью руководителя
\newcommand{\VSTUTitleDirector}{
\begin{minipage}[c]{15em}
Научный руководитель\\
\VSTUDirectorDegreeAndPost\\
\makebox[2cm]{\hrulefill} \VSTUDirectorName\\
<<\makebox[1.5cm]{\hrulefill}>> \makebox[3.5cm]{\hrulefill} \the\year
\end{minipage}
}

% Box с подписью нормоконтролера
\newcommand{\VSTUTitleStandardsAdviser}{
\begin{minipage}[c]{15em}
Нормоконтролер\\
\VSTUStandardsAdviserDegreeAndPost\\
\makebox[2cm]{\hrulefill} \VSTUStandardsAdviserName\\
<<\makebox[1.5cm]{\hrulefill}>> \makebox[3.5cm]{\hrulefill} \the\year
\end{minipage}
}

% Box с подписью исполнителя (студента)
\newcommand{\VSTUTitleImplementer}{
\begin{minipage}[c]{15em}
Исполнитель\\
студент~группы~\VSTUStudentGroup\\
\makebox[2cm]{\hrulefill} \VSTUStudentName\\
<<\makebox[1.5cm]{\hrulefill}>> \makebox[3.5cm]{\hrulefill} \the\year
\end{minipage}
}
